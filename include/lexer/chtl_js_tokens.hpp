...
/**
 * CHTL JS Token类型 - 严格按照语法文档中CHTL JS部分定义
 */
...
struct CHTLJSToken {
    ...
    CHTLJSToken(CHTLJSTokenType t, const std::string& v, Position pos, size_t len = 0)
...
enum class EnhancedSelectorType {
    ...
    AUTO_DETECT         // 自动检测 (先判断是否为tag，然后查找类名/id)
};
...
public:
    ...
    // 获取关键字对应的token类型
    CHTLJSTokenType getKeywordType(const std::string& word) const;
...
private:
    std::unordered_map<std::string, CHTLJSTokenType> keywordMap_;
...
public:
    ...
    static bool isLiteralType(CHTLJSTokenType type);
    static bool isKeywordType(CHTLJSTokenType type);
    static bool isOperatorType(CHTLJSTokenType type);
    static bool isSeparatorType(CHTLJSTokenType type);
    static bool isCommentType(CHTLJSTokenType type);
    static bool isWhitespaceType(CHTLJSTokenType type);
    static bool isCHTLJSSpecificType(CHTLJSTokenType type);
