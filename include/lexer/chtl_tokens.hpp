...
struct CHTLToken {
    ...
    CHTLToken(CHTLTokenType t, const std::string& v, Position pos, size_t len = 0)
...
private:
    std::unordered_map<std::string, CHTLTokenType> keywordMap_;
...
public:
    ...
    // 获取前缀对应的token类型
    CHTLTokenType getPrefixType(const std::string& prefix) const;
    
    // 根据配置更新前缀映射
    void updateFromConfiguration(const std::unordered_map<std::string, std::string>& config);
    
private:
    std::unordered_map<std::string, CHTLTokenType> prefixMap_;
...
public:
    ...
    static bool isLiteralType(CHTLTokenType type);
    static bool isKeywordType(CHTLTokenType type);
    static bool isOperatorType(CHTLTokenType type);
    static bool isSeparatorType(CHTLTokenType type);
    static bool isCommentType(CHTLTokenType type);
    static bool isWhitespaceType(CHTLTokenType type);
