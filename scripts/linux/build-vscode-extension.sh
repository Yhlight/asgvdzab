...
# VSCode CHTL Extension Build Script - Linux Platform
...
build_compiler() {
    ...
    if [ ! -f "target/chtl-compiler-1.0.jar" ]; then
        ...
        if [ -f "scripts/linux/build-compiler.sh" ]; then
            bash "scripts/linux/build-compiler.sh" --skip-tests
        else
...
copy_resources() {
    ...
    if [ -d "$PROJECT_ROOT/target/modules" ]; then
        ...
        if [ -d "$PROJECT_ROOT/src/main/java/com/chtl/module/Chtholly" ]; then
            bash "$PROJECT_ROOT/scripts/linux/package-unified.sh" \
...
show_build_info() {
    ...
    echo
    info "Installation:"
    echo "  Run: code --install-extension $VSIX_FILE"
    
    if [ -d "$OUTPUT_DIR" ] && [ -f "$OUTPUT_DIR/$VSIX_FILE" ]; then
        echo "  Or: code --install-extension $OUTPUT_DIR/$VSIX_FILE"
    fi
}
...
# Main execution
info "VSCode CHTL Extension Build Script"
info "=================================="
