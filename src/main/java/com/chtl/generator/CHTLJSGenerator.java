...
public class CHTLJSGenerator implements CHTLJSASTVisitor {
    ...
    private void generateCHTLHelper() {
        ...
        output.append("        // 提取目标元素\n");
        output.append("        const target = config.target || null;\n");
        output.append("        if (!target) {\n");
        output.append("            console.error('animate: 必须指定target元素');\n");
        output.append("            return null;\n");
        output.append("        }\n");
        output.append("        \n");
        output.append("        // 确保target是DOM元素\n");
        output.append("        const element = typeof target === 'string' ? \n");
        output.append("            document.querySelector(target) : target;\n");
        output.append("        if (!element) {\n");
        output.append("            console.error('animate: 无法找到目标元素', target);\n");
        output.append("            return null;\n");
        output.append("        }\n");
        output.append("        \n");
        output.append("        // 动画配置\n");
